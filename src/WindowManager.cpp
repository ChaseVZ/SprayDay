...
bool WindowManager::init(int const width, int const height)
{
	...
	glfwSetMouseButtonCallback(windowHandle, mouse_callback);  // >>>>> CHASE ADDED <<<<<<<
	glfwSetFramebufferSizeCallback(windowHandle, resize_callback);
	glfwSetScrollCallback(windowHandle, scroll_callback);
	glfwSetCursorPosCallback(windowHandle, cursor_callback); // >>>>> CHASE ADDED <<<<<<<
...
void WindowManager::mouse_callback(GLFWwindow* window, int button, int action, int mods) // >>>>> CHASE ADDED <<<<<<<
{
	if (instance && instance->callbacks)
	{
...
void WindowManager::cursor_callback(GLFWwindow* window, double xPos, double yPos) // >>>>> CHASE ADDED <<<<<<<
{
	if (instance && instance->callbacks)
	{
